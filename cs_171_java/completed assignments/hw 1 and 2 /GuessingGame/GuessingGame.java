...
public class GuessingGame {
	...
	//these store results for first 3 guesses to optimize guess selection
	...
	public int myGuessIs() {
		...
		if(numGuesses == 0) {
			...
			if (firstGuessNmatches == 0) {
				...
				for(int k = 0; k <= list.size()/2 && numEliminatedMax < 1811; k++) {
				...
				for(int k = 0; k <= list.size()/2 && numEliminatedMax < 189; k++) {
					int temp = numElim(list.get(k));
			...
			//if first 3 answers were 0 all guesses are equivalent
			if(firstGuessNmatches == 0 && secondGuessNmatches == 0 && thirdGuessNmatches == 0) {
				int g = r.nextInt(list.size());
