...
public class GuessingGame {
	...
	public int myGuessIs() {
		...
		if(numGuesses == 0) {
			...
			//will begin going forwards
			reverseDirection = false;
		}
		//implements a faster, less thorough version of the minimax guess optimization technique
		else if(numGuesses < 4) {
			int numEliminatedMax = -1;
			int currentGuess = -1;
			if(reverseDirection) {
				//will do each element for last 16th of the list 
				for(int k = list.size()-1; k >= (list.size()/16) * 15; k--) {
				...
				//will do each element for first quarter of list
				for(int k = 0; k <= list.size()/16; k++) {
					int temp = numElim(list.get(k));
			...
			int currentGuess = -1;
			if(reverseDirection) {
				//will do each element for last quarter of the list 
				for(int k = list.size()-1; k > (list.size()/4) * 3; k--) {
					int temp = numElim(list.get(k));
				...
				//increments by 3 for the next quarter
				for(int k = (list.size()/4)*3; k > list.size()/2; k = k - 3) {
					int temp = numElim(list.get(k));
				...
				//will increment by 3 for next quarter of list
				for(int k = list.size()/4; k < list.size()/2; k = k + 3) {
					int temp = numElim(list.get(k));
